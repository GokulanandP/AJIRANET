...
class Validator:
    ...
    def connector_data_validator(self,data,data_recieved):
        try:
            device_names = []
            for devices in data:
            ...
            if data_recieved['source'] not in device_names:
                ...
                self.return_data['message'] = "Node " + data_recieved['source'] + " not found"
                return False
            for targets in data_recieved['targets']:
                if targets not in device_names:
                    self.return_data['status'] = False
                    self.return_data['message'] = "Node " + targets + " not found"
                    return False
            return True
        except:
            self.return_data['status'] = False
            self.return_data['message'] = "Invalid command syntax"
            return False

    def Connector(self, data, data_recieved):
        if self.connector_data_validator(data,data_recieved):
            for targets in data_recieved['targets']:
                if data_recieved['source'] == targets:
                    self.return_data['status'] = False
                    self.return_data['message'] = "Cannot connect device to itself"
            ...
            for device in data:
                if device['name'] == data_recieved['source']:
                    if len(device['targets']) == 0:
                        ...
                        self.return_data['message'] = "Successfully connected"
                        return self.return_data
                    for i in data_recieved['targets']:
                        if i in device['targets']:
                            self.return_data['status'] = False
                            self.return_data['message'] = "Devices are already connected"
                            return self.return_data
    ...
    def check_path_data_validator(self, data, source, destination):
        if source == None or destination == None:
            self.return_data['status'] = False
            self.return_data['message'] = 'Invalid Request'
        device_names = []
        for devices in data:
            device_names.append(devices['name'])
        if source not in device_names:
            self.return_data['status'] = False
            self.return_data['message'] = "Node " + source + " not found"
            return False
        if destination not in device_names:
            self.return_data['status'] = False
            self.return_data['message'] = "Node " + destination + " not found"
            return False
        return True


    def Check_path(self, data, source, destination):
        if self.check_path_data_validator( data, source, destination):
            if source == destination:
                self.return_data['status'] = True
                self.return_data['message'] = 'Route '+ source + ' -> '+destination
                return self.return_data
            else:
                source_flag = False
                    ...
                    for device in data:
                        if device['name'] == source:
                            if destination in device['targets']:
                                self.return_data['status'] = True
                                self.return_data['message'] = 'The route is '+source+' -> '+destination
                                return self.return_data
                            route_map = [source]
                            self.route = source
                            for targets in device['targets']:
                                ...
                                if recursive_function_result:
                                    self.return_data['status'] = True
                                    self.return_data['message'] = 'Route is '+ self.route
                                    return self.return_data
                            ...
                            self.return_data['status'] = False
                            self.return_data['message'] = 'Route not found'
                            return self.return_data
                else:
                    self.return_data['status'] = False
                    self.return_data['message'] = 'Route cannot be calculated for repeater'
                    return self.return_data
        else:
            return self.return_data
